...
exports.EMPTY_HEIRS = void 0;
exports.calcHeirs = calcHeirs;
const won_1 = require("./won");
exports.EMPTY_HEIRS = { hasSpouse: true, children: 2, parents: 0, siblings: 0, estate: 10 * won_1.억 };
function gcd(a, b) {
    return b === 0 ? a : gcd(b, a % b);
}
function calcHeirs(input) {
    const steps = [];
    const tips = [];
    ...
    if (children > 0) {
        ...
        for (let i = 0; i < children; i++)
            units.push({ label: `자녀 ${i + 1}`, weight: 2, reserve: 0.5 });
        if (input.hasSpouse)
            units.push({ label: "배우자", weight: 3, reserve: 0.5 });
    }
    else if (parents > 0) {
        rank = `2순위 직계존속 ${parents}명${input.hasSpouse ? " + 배우자" : ""}`;
        for (let i = 0; i < parents; i++)
            units.push({ label: `직계존속 ${i + 1}`, weight: 2, reserve: 1 / 3 });
        if (input.hasSpouse)
            units.push({ label: "배우자", weight: 3, reserve: 0.5 });
    }
        ...
        for (let i = 0; i < siblings; i++)
            units.push({ label: `형제자매 ${i + 1}`, weight: 1, reserve: null });
    }
    else {
    ...
    if (estate > 0)
        steps.push({ label: "상속재산 (채무 공제 후)", value: estate, tone: "sub" });
    ...
    if (input.hasSpouse && (children > 0 || parents > 0)) {
        ...
        if (sp)
            tips.push({
                ...
                body: `배우자상속공제는 실제 상속받은 금액을 공제하되, 법정상속분 상당액(최대 30억원)이 상한입니다. 협의분할로 배우자 몫을 늘려도 이 선을 넘는 부분은 공제되지 않습니다.${estate > 0 ? ` 지금 재산 기준 ${(0, won_1.formatWon)(sp.amount)}.` : ""}`,
                law: "상속세 및 증여세법 제19조",
            });
    }
    tips.push({
