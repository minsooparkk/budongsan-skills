...
exports.EMPTY_GIFT = exports.RELATION_LABEL = exports.RELATIONS = void 0;
exports.relationDeductionCap = relationDeductionCap;
exports.calcGift = calcGift;
exports.calcGiftSplit = calcGiftSplit;
exports.giftDeadline = giftDeadline;
const progressive_1 = require("./progressive");
const validation_1 = require("./validation");
const won_1 = require("./won");
exports.RELATIONS = ["spouse", "lineal-desc", "lineal-asc", "kin", "other"];
exports.RELATION_LABEL = {
    spouse: "배우자",
...
/** 증여재산공제 한도 (§53) — 10년 통산 */
function relationDeductionCap(relation, minor) {
    switch (relation) {
        case "spouse":
            return 6 * won_1.억;
        case "lineal-desc":
            return minor ? 2000 * won_1.만 : 5000 * won_1.만;
        case "lineal-asc":
            return 5000 * won_1.만;
        case "kin":
            return 1000 * won_1.만;
...
function calcGift(input) {
    const invalid = (message) => (0, validation_1.invalidCalculation)({ payable: 0, taxBase: 0, grossTax: 0, effectiveRate: 0, deadline: "", belowMinimum: false, transferPortion: 0, priorCredit: 0, relationDeduction: 0, marriageDeduction: 0 }, message);
    if ((0, validation_1.hasInvalidNumbers)(input) || !(0, validation_1.isOneOf)(input.relation, exports.RELATIONS) || !(0, validation_1.isIntegerInRange)(input.month, 1, 12) || !(0, validation_1.isIntegerInRange)(input.year, 2024, 2026))
        return invalid("현재 검증된 2024~2026년 증여만 지원합니다. 증여 연·월, 관계와 금액을 확인해 주세요.");
    if (input.priorGiftTaxPaid > 0)
        return invalid("이전 링크의 실납부액은 납부세액공제 기준과 다릅니다. 이전 실납부액을 지우고 사전증여의 산출세액(신고세액공제 전)과 과세표준을 다시 입력해 주세요.");
    const priorGift = input.priorGift >= 1000 * won_1.만 ? input.priorGift : 0;
    if (priorGift > 0 && input.priorDetailsConfirmed !== true)
        return invalid("합산할 과거 증여의 과세표준·산출세액·공제 사용액을 신고서에서 확인해 주세요. 0원인 항목도 확인해야 합니다.");
    ...
    if (input.usedDeduction > relationCap || input.usedMarriageBirth > won_1.억 || (includedMarriage > 0 && input.relation !== "lineal-desc"))
        return invalid("선택한 증여 관계·미성년 여부와 과거 공제 이력이 맞지 않습니다. 혼인·출산 공제는 직계존속에게 받은 증여에만 적용되며, 관계를 바꾸면 이전 공제 이력도 다시 확인해 주세요.");
    if (input.generationSkip && input.relation !== "lineal-desc")
        return invalid("세대생략 할증은 조부모 등 직계존속에게서 받은 증여만 선택할 수 있습니다.");
    if (includedDeduction > input.usedDeduction || includedMarriage > input.usedMarriageBirth || includedDeduction + includedMarriage > priorGift || priorBase > priorGift || (priorGrossTax > 0 && priorBase <= 0))
        return invalid("합산 증여분의 공제액은 전체 사용 공제 이내여야 합니다. 사전증여의 과세표준·산출세액도 확인해 주세요.");
    if (input.generationSkip && priorGift > 0)
        return invalid("세대생략 증여를 합산할 때는 종전 할증세액과 증여자별 배분을 추가로 확인해야 합니다. 현재 간편 계산의 지원 범위를 벗어납니다.");
    const steps = [];
    const push = (s) => steps.push(s);
    /* ── 1. 증여재산 ─────────────────────────────────────────── */
    const gross = input.realEstate + input.cash + input.stock + input.etc;
    if (!Number.isSafeInteger(gross) || input.assumedDebt > gross)
        return invalid("증여재산·채무는 원 단위 정수여야 하고 인수 채무는 증여재산 합계를 넘을 수 없습니다.");
    push({ label: "증여재산가액", value: gross, tone: "sub", law: "상증세법 제60조" });
    if (input.realEstate)
        push({ label: "부동산", value: input.realEstate, depth: 1 });
    if (input.cash)
        push({ label: "현금", value: input.cash, depth: 1 });
    if (input.stock)
    ...
    push({
        ...
        note: cap === 0
            ...
            : `${exports.RELATION_LABEL[input.relation]} 한도 ${(0, won_1.formatWon)(cap)}${outsideDeduction ? ` 중 합산하지 않는 증여에 ${(0, won_1.formatWon)(outsideDeduction)}을 사용했다` : ""} — 10년 통산`,
    });
    ...
    if (input.marriageBirth && input.relation === "lineal-desc") {
        marriageDeduction = Math.max(0, Math.min(1 * won_1.억 - input.usedMarriageBirth + includedMarriage, taxableGift - relationDeduction));
    }
    ...
    push({ label: "적용세율", value: (0, won_1.pct)(applied.rate, 0), law: "상증세법 제56조" });
    ...
    if (input.generationSkip && baseTax > 0) {
        const rate = input.minor && gross > 20 * won_1.억 ? 0.4 : 0.3;
        surcharge = (0, won_1.decimalProduct)(baseTax, rate);
        push({
            label: `＋ 세대생략 할증 ${(0, won_1.pct)(rate, 0)}`,
            value: surcharge,
            tone: "plus",
            law: "상증세법 제57조",
            note: rate === 0.4
    ...
    const filingCredit = (0, won_1.decimalProduct)(afterCredit, 0.03);
    push({
        label: "− 신고세액공제 3%",
        value: filingCredit,
        tone: "minus",
    ...
    const payable = (0, won_1.floorProduct)([afterCredit, 0.97], 10);
    ...
    const headline = [
        { label: "납부할 증여세", value: payable, hint: `실효세율 ${(0, won_1.pct)(effectiveRate, 1)}` },
        { label: "과세표준", value: taxBase, hint: `적용세율 ${(0, won_1.pct)(applied.rate, 0)}` },
        { label: "증여재산공제", value: deduction, hint: `한도 ${(0, won_1.formatWon)(cap)}` },
    ];
    return {
        headline,
        steps,
 ...
 */
function calcGiftSplit(input) {
    const firstInput = { ...input };
    const secondInput = { ...input, priorGift: 0, priorGiftTaxPaid: 0, priorGiftGrossTax: 0, priorDetailsConfirmed: false, priorTaxBase: 0, priorIncludedDeduction: 0, priorIncludedMarriageBirth: 0, usedDeduction: 0, marriageBirth: false };
    for (const key of ["realEstate", "cash", "stock", "etc"]) {
    ...
    secondInput.usedMarriageBirth = Math.min(won_1.억, input.usedMarriageBirth + Math.max(0, first.marriageDeduction - (input.priorIncludedMarriageBirth ?? 0)));
...
function giftTips(input, r) {
    ...
    if (r.assumedDebt > 0) {
        tips.push({
            ...
            body: `채무 ${(0, won_1.formatWon)(r.assumedDebt)}만큼은 증여가 아니라 판 것으로 본다. 받는 사람은 그만큼 증여세가 줄지만, 주는 사람에게 그 부분의 양도소득세가 새로 생긴다. 두 세금을 합쳐야 유·불리가 나온다.`,
    ...
    if (input.relation !== "other") {
        tips.push({
            ...
            body: `증여재산공제 ${(0, won_1.formatWon)(r.cap)}은 한 번 쓰면 10년이 지나야 다시 생긴다. 반대로 10년 간격을 두고 나눠 주면 낮은 누진 구간을 두 번 쓴다 — 증여를 일찍 시작할수록 유리한 이유다.`,
            law: "상증세법 제53조",
        });
    }
    ...
    if (r.payable > 20_000_000) {
        tips.push({
            level: "save",
            title: "연부연납 — 최장 5년",
            body: `납부세액이 2천만원을 넘으면 담보를 걸고 나눠 낼 수 있다. 지금 세액 ${(0, won_1.formatWon)(r.payable)}이면 대상이다. 가산금(이자)이 붙는다.`,
            law: "상증세법 제71조",
        });
    }
    tips.push({
