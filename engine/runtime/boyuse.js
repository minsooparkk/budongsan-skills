...
exports.CASES = exports.URBAN_AREA_RATE = exports.PROPERTY_FMV = exports.REGIME_LABEL = void 0;
exports.ageCreditRate = ageCreditRate;
exports.periodCreditRate = periodCreditRate;
exports.creditRateOf = creditRateOf;
exports.calculateCase = calculateCase;
exports.calculateAll = calculateAll;
exports.awayPenalty = awayPenalty;
exports.formatWon = formatWon;
exports.formatEok = formatEok;
const 억 = 100_000_000;
exports.REGIME_LABEL = {
    current: "현행",
    y2027: "2027년 (중간)",
    y2028: "2028년~ (최종)",
};
...
function bandOf(base, regime) {
    const bands = regime === "current" ? BANDS_CURRENT : regime === "y2027" ? BANDS_2027 : BANDS_2028;
 ...
 */
function propertyTax(taxBase) {
    if (taxBase <= 60_000_000)
        return taxBase * 0.001;
    if (taxBase <= 1.5 * 억)
        return taxBase * 0.0015 - 30_000;
    if (taxBase <= 3 * 억)
        return taxBase * 0.0025 - 180_000;
    return taxBase * 0.004 - 630_000;
}
/** 재산세 공정시장가액비율 — 1세대 1주택 특례 45% */
exports.PROPERTY_FMV = 0.45;
/**
 ...
 */
exports.URBAN_AREA_RATE = 0.0014;
exports.CASES = [
 ...
 */
function asSingleHouseholder(c) {
 ...
 */
function taxableThreshold(c, regime) {
    // 현행은 별도 문턱 없이 기본공제가 곧 경계다
    if (regime === "current")
        return deduction(c, regime);
 ...
 */
function deduction(c, regime) {
    if (regime === "current") {
        // 현행: 1세대1주택(특례 포함) 12억, 부부 개별 납부는 각 9억 (거주 무관)
        return asSingleHouseholder(c) ? 12 * 억 : 9 * 억;
    }
    if (asSingleHouseholder(c))
        return c.resident ? 14 * 억 : 12 * 억;
 ...
 */
function periodCreditRate(holdYears, liveYears, regime) {
    if (regime === "current")
        return periodRateFull(holdYears);
    if (regime === "y2027") {
...
const CREDIT_AMOUNT_CAP = {
    current: Infinity,
    y2027: 8_000_000,
    y2028: 6_000_000,
};
function creditRateOf(p, regime) {
    return Math.min(CREDIT_RATE_CAP, ageCreditRate(p.age) + periodCreditRate(p.holdYears, p.liveYears, regime));
}
function calculateCase(c, p, regime) {
    ...
    const overlapBase = taxBase * exports.PROPERTY_FMV;
    ...
    const propBase = p.price * exports.PROPERTY_FMV; // 재산세는 주택 전체 기준
    const prop = propertyTax(propBase);
    const urban = propBase * exports.URBAN_AREA_RATE;
...
function calculateAll(p, regime) {
    return Object.fromEntries(exports.CASES.map((c) => [c.key, calculateCase(c, p, regime)]));
}
/** 같은 명의에서 비거주가 실거주보다 얼마나 더 내는지 */
function awayPenalty(results, joint) {
