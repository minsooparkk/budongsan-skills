...
exports.EMPTY_REBUILD = exports.EMPTY_COVERAGE = exports.EMPTY_LAND_SHARE = exports.EMPTY_UNIT_PRICE = exports.EMPTY_LENGTH = exports.LENGTH_LABEL = exports.LENGTH_UNITS = exports.EMPTY_AREA = exports.EMPTY_DATE = exports.PYEONG = void 0;
exports.calcDate = calcDate;
exports.calcArea = calcArea;
exports.calcLength = calcLength;
exports.calcUnitPrice = calcUnitPrice;
exports.calcLandShare = calcLandShare;
exports.calcCoverage = calcCoverage;
exports.calcRebuild = calcRebuild;
const validation_1 = require("./validation");
const won_1 = require("./won");
exports.PYEONG = 400 / 121; // 3.3058㎡
exports.EMPTY_DATE = { from: "2024-03-15", to: "2026-09-03" };
function calcDate(input) {
    const steps = [];
    const tips = [];
    const a = (0, validation_1.parseDateOnly)(input.from);
    const b = (0, validation_1.parseDateOnly)(input.to);
    if (!a || !b || b.getTime() < a.getTime())
        return (0, validation_1.invalidCalculation)({ days: 0, years: 0, months: 0, rest: 0 }, "유효한 시작일·종료일을 입력해 주세요. 종료일은 시작일보다 빠를 수 없습니다.");
    ...
    if (addMonths(total).getTime() > b.getTime())
        total--;
...
exports.EMPTY_AREA = { value: 84, unit: "sqm" };
function calcArea(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || !(0, validation_1.isOneOf)(input.unit, ["sqm", "pyeong", "sqft"]))
        return (0, validation_1.invalidCalculation)({ sqm: 0, pyeong: 0, sqft: 0 }, "면적과 변환 단위를 확인해 주세요.");
    const sqm = input.unit === "sqm" ? input.value : input.unit === "pyeong" ? input.value * exports.PYEONG : input.value * 0.09290304;
    const pyeong = sqm / exports.PYEONG;
...
/* ═══════════ 길이 환산 ═══════════ */
exports.LENGTH_UNITS = { m: 1, km: 1000, cm: 0.01, mm: 0.001, inch: 0.0254, ft: 0.3048, yd: 0.9144, mile: 1609.344, ja: 10 / 33, ri: 392.727 };
exports.LENGTH_LABEL = { m: "미터 m", km: "킬로미터 km", cm: "센티미터 cm", mm: "밀리미터 mm", inch: "인치 in", ft: "피트 ft", yd: "야드 yd", mile: "마일 mi", ja: "자 (尺)", ri: "리 (里)" };
exports.EMPTY_LENGTH = { value: 100, unit: "m" };
function calcLength(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || !(0, validation_1.isOneOf)(input.unit, Object.keys(exports.LENGTH_UNITS)))
        return (0, validation_1.invalidCalculation)({ meters: 0 }, "길이와 변환 단위를 확인해 주세요.");
    const meters = input.value * exports.LENGTH_UNITS[input.unit];
    const fmt = (u) => { const v = meters / exports.LENGTH_UNITS[u]; return v >= 1000 ? v.toLocaleString("ko-KR", { maximumFractionDigits: 2 }) : v.toLocaleString("ko-KR", { maximumFractionDigits: 4 }); };
    const steps = Object.keys(exports.LENGTH_UNITS).map((u) => ({ label: exports.LENGTH_LABEL[u], value: fmt(u), tone: u === input.unit ? "sub" : undefined }));
    const tips = [{ level: "watch", title: "1자 = 10/33m, 1리 ≈ 393m", body: "옛 도면·등기의 척관법 단위입니다. 1평 = 6자 × 6자 = 36자² = 400/121㎡가 여기서 나옵니다." }];
...
exports.EMPTY_UNIT_PRICE = { price: 12 * 10 ** 8, area: 84, unit: "sqm" };
function calcUnitPrice(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || input.area <= 0 || !(0, validation_1.isOneOf)(input.unit, ["sqm", "pyeong"]))
        return (0, validation_1.invalidCalculation)({ perSqm: 0, perPyeong: 0 }, "0보다 큰 면적과 유효한 면적 단위를 입력해 주세요.");
    const sqm = input.unit === "sqm" ? input.area : input.area * exports.PYEONG;
    const perSqm = sqm > 0 ? input.price / sqm : 0;
    const perPyeong = perSqm * exports.PYEONG;
    const steps = [
        { label: "가격", value: input.price, tone: "sub" },
        { label: "면적", value: `${sqm.toFixed(2)}㎡ = ${(sqm / exports.PYEONG).toFixed(2)}평` },
    ...
    return { headline: [{ label: "평당 가격", value: Math.floor(perPyeong) }, { label: "㎡당", value: Math.floor(perSqm) }, { label: "면적", value: 0, displayValue: `${(sqm / exports.PYEONG).toFixed(1)}평` }], steps, tips, perSqm, perPyeong };
}
exports.EMPTY_LAND_SHARE = { siteArea: 12000, myExclusive: 84, totalExclusive: 40000, landPrice: 10_000_000 };
function calcLandShare(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || input.totalExclusive <= 0 || input.myExclusive > input.totalExclusive)
        return (0, validation_1.invalidCalculation)({ share: 0, shareRatio: 0, value: 0 }, "전체 전용면적은 0보다 커야 하고 내 전용면적 이상이어야 합니다.");
    ...
    const steps = [
        ...
        { label: `× 내 전용면적 ÷ 전체 전용면적 (${input.myExclusive} ÷ ${input.totalExclusive.toLocaleString()} = ${(shareRatio * 100).toFixed(4)}%)`, value: `${share.toFixed(2)}㎡ = ${(share / exports.PYEONG).toFixed(2)}평`, tone: "total", law: "집합건물법 제12조 (전유부분 면적 비율)" },
        { label: `× 공시지가 ${(0, won_1.formatWon)(input.landPrice)}/㎡ = 대지지분 가액`, value },
    ...
    return { headline: [{ label: "대지지분", value: 0, displayValue: `${share.toFixed(2)}㎡ = ${(share / exports.PYEONG).toFixed(2)}평` }, { label: "대지지분 가액 (공시지가)", value }, { label: "지분 비율", value: 0, displayValue: `${(shareRatio * 100).toFixed(4)}%` }], steps, tips, share, shareRatio, value };
}
exports.EMPTY_COVERAGE = { siteArea: 300, buildingArea: 170, floorArea: 600, maxCoverage: 0.6, maxFar: 2.0 };
function calcCoverage(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || input.siteArea <= 0)
        return (0, validation_1.invalidCalculation)({ coverage: 0, far: 0, maxBuildingArea: 0, maxFloorArea: 0 }, "0보다 큰 대지면적과 유효한 건축면적을 입력해 주세요.");
    ...
    const maxBuildingArea = input.siteArea * input.maxCoverage;
    const maxFloorArea = input.siteArea * input.maxFar;
    const steps = [
    ...
    if (coverage > input.maxCoverage || far > input.maxFar)
        tips.unshift({ level: "must", title: "법정 상한을 넘습니다", body: "건축허가가 나지 않습니다. 건축면적이나 연면적을 줄이거나, 용적률 인센티브(공개공지·임대주택)를 검토하세요." });
...
exports.EMPTY_REBUILD = { completedYear: 1998, limitYears: 30, safetyPassed: false };
function calcRebuild(input) {
