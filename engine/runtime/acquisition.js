...
exports.EMPTY_ACQUISITION = exports.KIND_LABEL = exports.KINDS = exports.CAUSE_LABEL = exports.CAUSES = exports.국민주택규모 = exports.중과기준세율 = void 0;
exports.houseRate = houseRate;
exports.heavyMultiple = heavyMultiple;
exports.calcAcquisition = calcAcquisition;
...
/** 국민주택규모 — 농특세 비과세 경계 */
exports.국민주택규모 = 85;
exports.CAUSES = ["purchase", "gift", "inherit", "original"];
exports.CAUSE_LABEL = {
    purchase: "매매 (유상)",
    gift: "증여 (무상)",
    inherit: "상속",
...
exports.KINDS = ["house", "land-farm", "building"];
exports.KIND_LABEL = {
    house: "주택",
    building: "주택 외 건물·토지 (상가·오피스텔 등)",
    "land-farm": "농지",
};
exports.EMPTY_ACQUISITION = {
    cause: "purchase",
    kind: "house",
    price: 9 * won_1.억,
    area: 84,
 ...
 */
function houseRate(price) {
    if (!(0, validation_1.isFiniteNumber)(price))
        return 0;
    if (price <= 6 * won_1.억)
        return 0.01;
    if (price > 9 * won_1.억)
        return 0.03;
    const numerator = (BigInt(Math.round(price)) * BigInt(2) - BigInt(9 * won_1.억)) * BigInt(10_000);
    return Number((numerator + BigInt(1.5 * won_1.억)) / BigInt(3 * won_1.억)) / 1_000_000;
}
/** 중과 판정 — 지방세법 제13조의2 제1항. 반환값은 중과기준세율의 배수(200% / 400%) */
function heavyMultiple(input) {
    if (input.kind !== "house" || input.cause !== "purchase")
        return 0;
    if (input.corporate)
        return 4;
...
function calcAcquisition(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || !(0, validation_1.isOneOf)(input.cause, exports.CAUSES) || !(0, validation_1.isOneOf)(input.kind, exports.KINDS) || !(0, validation_1.isIntegerInRange)(input.houses, 1, 10000))
        return (0, validation_1.invalidCalculation)({ rate: 0, heavy: null, acquisitionTax: 0, eduTax: 0, ruralTax: 0, discount: 0, total: 0, effectiveRate: 0, deadlineLabel: "" }, "취득 원인·종류와 금액, 주택 수(1~10,000채 정수)를 확인해 주세요.");
    const steps = [];
    ...
    const firstHomeEligible = input.firstHome &&
        !input.corporate &&
    ...
    if (input.cause === "purchase") {
        if (input.kind === "house") {
            if (heavyApplies) {
                rate = 0.04 + exports.중과기준세율 * multiple;
        ...
        const giftHeavy = input.kind === "house" && input.regulated && price >= 3 * won_1.억;
        if (giftHeavy) {
            rate = 0.12;
    ...
    push({
        label: "적용 취득세율",
        value: (0, won_1.pct)(rate, 4),
        law: rateLaw,
        note: rateNote || undefined,
    });
    const grossAcquisition = (0, won_1.decimalProduct)(price, rate);
    push({ label: "취득세 산출세액", value: grossAcquisition, tone: "sub" });
    /* ── 2. 생애최초 감면 (지특법 §36의3) ───────────────────── */
    let discount = 0;
    if (firstHomeEligible) {
        const cap = input.firstHomeSmall ? 300 * won_1.만 : 200 * won_1.만;
        ...
        push({
            ...
            tone: "minus",
            law: "지방세특례제한법 제36조의3",
            note: `산출세액이 ${(0, won_1.formatWon)(cap)} 이하면 전액 면제, 넘으면 ${(0, won_1.formatWon)(cap)}을 뺀다 (2028년 말까지)`,
        });
    }
    const acquisitionTax = discount >= grossAcquisition ? 0 : Math.max(0, (0, won_1.floorProduct)([price, rate], 10) - discount);
    ...
    if (heavyApplies || (input.cause === "gift" && heavy)) {
        ...
        eduBaseRate = (0, won_1.decimalProduct)(rate, 0.1);
    }
    ...
    let eduTax = (0, won_1.floorProduct)([price, eduBaseRate], 10);
    // 감면을 받으면 지방교육세도 같은 비율로 깎인다 (§151①1 다목 1))
    if (discount > 0 && grossAcquisition > 0) {
        // 생애최초 주택 감면: 같은 비율로 교육세 감면 → 남은 본세의 10%.
        eduTax = (0, won_1.floorProduct)([Math.max(0, grossAcquisition - discount), 0.1], 10);
    }
    push({
        label: "지방교육세",
        value: eduTax,
        law: eduLaw,
        note: `${(0, won_1.pct)(eduBaseRate, 5)} — ${input.cause === "purchase" && input.kind === "house" && !heavyApplies
            ? "주택 유상거래는 취득세율의 10%"
    ...
    const exemptSmallHouse = input.kind === "house" && input.area > 0 && input.area <= exports.국민주택규모;
    ...
    const ruralOnDiscount = !exemptRural && discount > 0 ? (0, won_1.decimalProduct)(discount, 0.2) : 0;
    const ruralTax = (0, won_1.truncate10)((0, won_1.floorProduct)([price, ruralRate]) + ruralOnDiscount);
    push({
        label: "농어촌특별세",
        value: exemptRural
        ...
        note: exemptRural
            ? undefined
            : `${(0, won_1.pct)(ruralRate)}${ruralOnDiscount ? " + 감면세액의 20%" : ""}`,
    });
    ...
    const deadlineLabel = input.cause === "inherit"
        ? "상속개시일이 속하는 달의 말일부터 6개월"
        : input.cause === "gift"
            ? "취득일이 속하는 달의 말일부터 3개월"
            : "취득일부터 60일";
    const headline = [
        { label: "총 납부세액", value: total, hint: `실효세율 ${(0, won_1.pct)(effectiveRate)}` },
        { label: "취득세", value: acquisitionTax, hint: `세율 ${(0, won_1.pct)(rate, 4)}` },
...
function acquisitionTips(input, r) {
    const tips = [];
    tips.push({
        level: "must",
    ...
    if (!input.firstHome && input.cause === "purchase" && input.kind === "house" && r.price <= 12 * won_1.억) {
        tips.push({
            level: "save",
    ...
    if (input.kind === "house" && input.area > exports.국민주택규모) {
        tips.push({
            level: "watch",
