...
exports.EMPTY_APPRAISAL = exports.APPRAISAL_METHOD_LABEL = exports.APPRAISAL_METHODS = exports.EMPTY_BUILDING_VAT = exports.EMPTY_RESIDUAL = exports.EMPTY_BUILDING_VALUE = exports.EMPTY_REMODEL = exports.EMPTY_TOTAL_INCOME = exports.EMPTY_FAIR = void 0;
exports.calcFairPrice = calcFairPrice;
exports.calcTotalIncome = calcTotalIncome;
exports.calcRemodel = calcRemodel;
exports.calcBuildingValue = calcBuildingValue;
exports.calcResidual = calcResidual;
exports.calcBuildingVat = calcBuildingVat;
exports.calcAppraisal = calcAppraisal;
const bands_1 = require("./bands");
const property_1 = require("./property");
const validation_1 = require("./validation");
const won_1 = require("./won");
exports.EMPTY_FAIR = { monthly: 200 * won_1.만, deposit: 5000 * won_1.만, targetYield: 0.05, annualExpenses: 300 * won_1.만, vacancy: 0.05, acquisitionRate: 0.05, extraCost: 0 };
function calcFairPrice(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || input.targetYield <= 0 || !(0, validation_1.isFiniteNumber)(input.vacancy, 0, 1))
        return (0, validation_1.invalidCalculation)({ noi: 0, price: 0 }, "목표수익률은 0보다 커야 하며, 비용은 0 이상, 공실률은 0~100%여야 합니다.");
    const steps = [];
    const tips = [];
    ...
    if (input.extraCost)
        steps.push({ label: "− 명도·수리 등 추가비용", value: input.extraCost, tone: "minus" });
...
exports.EMPTY_TOTAL_INCOME = { earned: 5000 * won_1.만, business: 0, rental: 1500 * won_1.만, other: 0, dependents: 1, deductions: 300 * won_1.만, credits: 0, prepaid: 0, hasEarned: true };
function calcTotalIncome(input) {
    const steps = [];
    const tips = [];
    const income = Math.max(0, input.earned) + Math.max(0, input.business) + Math.max(0, input.rental) + Math.max(0, input.other);
    const basic = Math.max(1, input.dependents) * 150 * won_1.만;
    const taxBase = Math.max(0, income - basic - input.deductions);
    const { tax: raw, band } = (0, property_1.applyStatute)(taxBase, bands_1.INCOME_BANDS);
    const tax = (0, won_1.truncate10)(raw);
    const standard = input.credits > 0 ? 0 : input.hasEarned ? 13 * won_1.만 : 7 * won_1.만;
    const determined = Math.max(0, tax - input.credits - standard);
    const local = (0, won_1.truncate10)(determined * 0.1);
    ...
    steps.push({ label: input.credits > 0 ? "− 세액공제·감면" : `− 표준세액공제 ${(0, won_1.formatWon)(standard)}`, value: input.credits > 0 ? input.credits : standard, tone: "minus", law: "소득세법 제59조의4 제9항" });
    ...
    if (input.rental > 0 && input.rental <= 2000 * won_1.만 * 0.5)
        tips.push({ level: "save", title: "주택임대 총수입 2천만 이하면 분리과세와 비교하세요", body: "종합과세에 넣지 않고 14% 분리과세를 고를 수 있습니다. 주택임대소득세 계산기에서 둘을 비교합니다.", law: "소득세법 제64조의2" });
...
exports.EMPTY_REMODEL = { cost: 5000 * won_1.만, rentIncrease: 30 * won_1.만, valueIncrease: 3000 * won_1.만, years: 5, rate: 0.05 };
function calcRemodel(input) {
    const steps = [];
    const tips = [];
    ...
    if (net < 0)
        tips.push({ level: "must", title: "이 조건으로는 손해입니다", body: "월세 증가나 가치 상승 가정을 낮춰 보수적으로 넣었는데도 마이너스면, 리모델링보다 매각이나 현상 유지가 낫습니다." });
...
exports.EMPTY_BUILDING_VALUE = { area: 100, unitPrice: 830_000, structureIndex: 1, useIndex: 1, locationIndex: 1, residualRate: 0.8 };
function calcBuildingValue(input) {
    const steps = [];
    const tips = [];
...
exports.EMPTY_RESIDUAL = { newCost: 5 * won_1.억, age: 15, life: 40, salvage: 0.1, method: "straight" };
function calcResidual(input) {
    if ((0, validation_1.hasInvalidNumbers)(input) || !(0, validation_1.isIntegerInRange)(input.life, 1, 500) || !(0, validation_1.isFiniteNumber)(input.salvage, 0, 1) || !(0, validation_1.isOneOf)(input.method, ["straight", "declining"]))
        return (0, validation_1.invalidCalculation)({ rate: 0, value: 0, depreciation: 0 }, "내용연수는 1~500년 정수, 잔가율은 0~100%여야 합니다. 감가 방식을 확인해 주세요.");
    const steps = [];
    const tips = [];
    const t = Math.min(input.life, Math.max(0, input.age));
    let rate;
    if (input.method === "straight")
        rate = 1 - (1 - input.salvage) * (t / input.life);
    else
        rate = Math.pow(input.salvage, t / input.life);
...
exports.EMPTY_BUILDING_VAT = { price: 10 * won_1.억, landStd: 5 * won_1.억, buildingStd: 2 * won_1.억, contractBuilding: 0, buyerBusiness: true };
function calcBuildingVat(input) {
    const steps = [];
    const tips = [];
    ...
    if (input.contractBuilding > 0)
        steps.push({ label: contractOk ? "계약서 건물가액 (안분가액과 30% 이내 — 인정)" : "계약서 건물가액 (안분가액과 30% 이상 차이 — 부인, 안분가액 적용)", value: input.contractBuilding, law: "부가가치세법 제29조 제9항" });
...
/* ═══════════ 감정평가 8방식 ═══════════ */
exports.APPRAISAL_METHODS = ["cost", "sales", "land", "income", "dcf", "rent-sales", "rent-cost", "rent-income"];
exports.APPRAISAL_METHOD_LABEL = { cost: "원가법 (재조달원가 − 감가)", sales: "거래사례비교법", land: "공시지가기준법", income: "수익환원법 (한 해 수익 ÷ 환원율)", dcf: "미래 수익의 현재가치 (DCF)", "rent-sales": "임대사례비교법", "rent-cost": "적산법 (기초가액 × 기대이율)", "rent-income": "수익분석법" };
exports.EMPTY_APPRAISAL = { method: "income", landValue: 5 * won_1.억, newCost: 3 * won_1.억, residualRate: 0.7, caseValue: 8 * won_1.억, timeFactor: 1.03, areaFactor: 1, conditionFactor: 1, otherFactor: 1, officialPrice: 500 * won_1.만, area: 200, timeRate: 1.02, regionFactor: 1, itemFactor: 1.1, noi: 4000 * won_1.만, capRate: 0.05, years: 5, growth: 0.02, discountRate: 0.06, exitCap: 0.055, baseValue: 8 * won_1.억, expectedRate: 0.04, expenses: 500 * won_1.만, businessProfit: 1 * won_1.억, rentShare: 0.3 };
function calcAppraisal(input) {
    if (!(0, validation_1.isOneOf)(input.method, exports.APPRAISAL_METHODS) || Object.entries(input).some(([key, value]) => typeof value === "number" && !(0, validation_1.isFiniteNumber)(value, key === "growth" ? -0.99 : 0)) || (input.method === "dcf" && (!(0, validation_1.isIntegerInRange)(input.years, 1, 100) || input.exitCap <= 0 || input.discountRate > 1 || input.growth > 1)) || (input.method === "income" && input.capRate <= 0))
        return (0, validation_1.invalidCalculation)({ value: 0, isRent: false }, "평가 방식과 금액·요율을 확인해 주세요. DCF 기간은 1~100년 정수이며 환원율은 0보다 커야 합니다.");
    const steps = [];
    const tips = [];
    let value = 0;
    let isRent = false;
    ...
    if (m === "cost") {
        ...
        for (let y = 1; y <= input.years; y++) {
            if (y > 1)
                cf = cf * (1 + input.growth);
            pv += cf / Math.pow(1 + input.discountRate, y);
        }
    ...
    if (!(0, validation_1.isFiniteNumber)(value))
        return (0, validation_1.invalidCalculation)({ value: 0, isRent }, "계산 결과가 지원 범위를 넘습니다. 금액·배율·기간을 줄여 주세요.");
    return { headline: [{ label: `${exports.APPRAISAL_METHOD_LABEL[m].split(" ")[0]} ${isRent ? "임료 (연)" : "가액"}`, value }, ...(isRent ? [{ label: "월 환산", value: Math.floor(value / 12) }] : [])], steps, tips, value, isRent };
}
