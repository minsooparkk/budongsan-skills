...
import { applyBands, BANDS_INCOME } from "./progressive";
...
export const EMPTY_TRANSFER: TransferInput = {
  single: true,
  houses: 1,
  regulated: true,
  salePrice: 30 * 억,
...
export function table1(hold: number): number {
  if (hold < 3) return 0;
  return Math.min(hold, 15) * 0.02;
}

/** 표2 — 1세대1주택 보유 + 거주. 각 최대 40%, 합 최대 80%. §95② 표2 */
export function table2(hold: number, live: number): number {
  if (hold < 3) return 0;
  // 시행령 §159의4 — 표2는 「보유기간 중 거주기간이 2년 이상인」 1세대1주택에만. 거주 2년 미만은 표1
  if (live < 2) return table1(hold);
 ...
 */
export function deductionRateOf(
  year: Year,
  regime: Regime,
  single: boolean,
  hold: number,
...
export function deductionCapOf(year: Year, regime: Regime): number | null {
  if (regime !== "reform") return null;
  if (year === 2028) return 20 * 억;
  if (year >= 2029) return 10 * 억;
  return null;
}
 ...
 */
export function surchargeOf(year: Year, regime: Regime, houses: number, hold = 2): number {
  if (houses < 2) return 0;
  const three = houses >= 3;
  const full = three ? 0.3 : 0.2;
  if (regime !== "reform" || hold < 2) return full;
  if (year === 2026 || year === 2027) return three ? 0.1 : 0.05;
  if (year === 2028) return three ? 0.15 : 0.1;
  return full; // 2029년 원상복귀
}

/** 양도소득 기본공제 — 개편안은 2027년부터 장기거주 1주택에 2,500만원 */
export function basicDeductionOf(
  year: Year,
  regime: Regime,
  single: boolean,
  live: number,
  ...
  if (regime === "reform" && year >= 2027 && single && live >= 10 && salePrice <= 30 * 억) {
    return 2500 * 만;
  }
  return 250 * 만;
}
...
export function calcYear(input: TransferInput, year: Year, regime: Regime): YearResult {
  const steps: Step[] = [];
  ...
  const exempt = input.single && input.salePrice <= 고가주택기준 && hold >= 2;
  ...
  if (input.single && hold >= 2) {
    if (exempt) {
      ...
      const ratio = (input.salePrice - 고가주택기준) / input.salePrice;
      ...
      push({
        ...
        tone: "sub",
        law: "소득세법 시행령 제160조",
        note: `양도차익 × (양도가액 − 12억) ÷ 양도가액 = ${pct(ratio, 1)}만 과세된다`,
      });
    }
  }
  ...
  push({
    ...
    note: deductionExcluded
      ...
      : `공제율 ${pct(deductionRate, 0)} (보유 ${hold}년 · 거주 ${live}년)${
          capApplied ? ` — ${formatWon(deductionCap!)} 한도에서 잘림` : ""
        }`,
  });
  ...
  push({
    ...
    tone: "minus",
    law: basicDeduction > 250 * 만 ? "2026년 세제개편안" : "소득세법 제103조",
    note:
      basicDeduction > 250 * 만
  ...
  const applied = applyBands(taxBase, BANDS_INCOME);
  ...
  push({
    label: "적용세율",
    value: surcharge > 0 ? `${pct(applied.rate, 0)} ＋ ${pct(surcharge, 0)}p` : pct(applied.rate, 0),
  ...
  const transferTax = truncate10(Math.max(0, taxBase * rate - applied.quick));
  push({ label: "양도소득세", value: transferTax, tone: "sub" });

  const localTax = truncate10(transferTax * 0.1);
  push({
    label: "＋ 개인지방소득세 10%",
    value: localTax,
    tone: "plus",
...
export function calcTransfer(input: TransferInput): TransferResult {
  const actual = YEARS.map((y) => calcYear(input, y, "reform"));
  const current = YEARS.map((y) => calcYear(input, y, "current"));
...
function transferTips(input: TransferInput, actual: YearResult[]): Tip[] {
  const tips: Tip[] = [];
  ...
  if (gap > 0) {
    tips.push({
      level: "must",
      title: `2029년에 팔면 2026년보다 ${formatWon(gap)}을 더 낸다`,
  ...
  if (capped) {
    tips.push({
      ...
      body: `공제율로는 ${formatWon(
        capped.taxableGain * capped.deductionRate,
      )}이 빠져야 하지만 한도 ${formatWon(
        capped.deductionCap!,
      )}에서 잘린다. 한도는 사람별·연간·물건별로 각각 적용되므로, 물건을 나눠 여러 해에 걸쳐 파는 설계가 의미를 갖는다.`,
      law: "2026년 세제개편안",
    });
  }
