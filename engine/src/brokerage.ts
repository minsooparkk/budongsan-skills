...
import { hasInvalidNumbers, invalidCalculation, isOneOf, type CalculationValidation } from "./validation";
import { formatWon, 만, 억 } from "./won";

export const BROKERAGE_KINDS = ["house", "officetel-home", "other"] as const;
export type BrokerageKind = (typeof BROKERAGE_KINDS)[number];
export const BROKERAGE_KIND_LABEL: Record<BrokerageKind, string> = {
  house: "주택 (부속토지 포함)",
  "officetel-home": "주거용 오피스텔",
  other: "그 밖의 부동산 (상가·토지·일반 오피스텔)",
};

export const DEALS = ["sale", "lease"] as const;
export type Deal = (typeof DEALS)[number];
export const DEAL_LABEL: Record<Deal, string> = { sale: "매매·교환", lease: "임대차" };
...
export const EMPTY_BROKERAGE: BrokerageInput = {
  kind: "house",
  deal: "sale",
  price: 9 * 억,
  monthly: 0,
...
export const HOUSE_SALE: Band[] = [
  { below: 5000 * 만, rate: 0.006, cap: 25 * 만 },
  { below: 2 * 억, rate: 0.005, cap: 80 * 만 },
  { below: 9 * 억, rate: 0.004, cap: Infinity },
  { below: 12 * 억, rate: 0.005, cap: Infinity },
  { below: 15 * 억, rate: 0.006, cap: Infinity },
  { below: Infinity, rate: 0.007, cap: Infinity },
];

/** 주택 임대차 — 별표 1 제1호 나목 */
export const HOUSE_LEASE: Band[] = [
  { below: 5000 * 만, rate: 0.005, cap: 20 * 만 },
  { below: 1 * 억, rate: 0.004, cap: 30 * 만 },
  { below: 6 * 억, rate: 0.003, cap: Infinity },
  { below: 12 * 억, rate: 0.004, cap: Infinity },
  { below: 15 * 억, rate: 0.005, cap: Infinity },
  { below: Infinity, rate: 0.006, cap: Infinity },
];

/** 주거용 오피스텔(전용 85㎡ 이하, 부엌·화장실·목욕시설) — 별표 1 제2호 */
export const OFFICETEL_RATE: Record<Deal, number> = { sale: 0.005, lease: 0.004 };

/** 그 밖의 부동산 — 별표 1 제3호. 0.9% 이내 협의 */
export const OTHER_MAX_RATE = 0.009;

/**
 * 임대차 거래금액 — 시행규칙 제20조 제5항.
 * 보증금 + 월세 × 100. 그 합이 5천만원 미만이면 보증금 + 월세 × 70 으로 다시 계산한다.
 */
export function leaseDealAmount(deposit: number, monthly: number): { amount: number; multiplier: 100 | 70 } {
  const by100 = deposit + monthly * 100;
  if (by100 < 5000 * 만) return { amount: deposit + monthly * 70, multiplier: 70 };
...
export function calcBrokerage(input: BrokerageInput): BrokerageResult {
  if (hasInvalidNumbers(input) || !isOneOf(input.kind, BROKERAGE_KINDS) || !isOneOf(input.deal, DEALS)) return invalidCalculation({ dealAmount: 0, rate: 0, cap: 0, fee: 0, vat: 0, total: 0, maxRate: 0 }, "부동산 종류·거래 방식과 금액을 확인해 주세요.");
  const steps: Step[] = [];
  const tips: Tip[] = [];
  ...
  if (input.deal === "lease") {
    ...
    steps.push({
      ...
      note:
        monthly > 0
          ? `보증금 ${formatWon(price)} + 월세 ${formatWon(monthly)} × ${multiplier}${
              multiplier === 70 ? " (100배 환산액이 5천만원 미만이라 70배로 다시 계산)" : ""
            }`
          : "월세가 없으면 보증금이 곧 거래금액",
    });
  } else {
  ...
  if (input.kind === "house" && input.deal === "sale") {
    const edges = [9 * 억, 12 * 억, 15 * 억];
    const near = edges.find((e) => dealAmount >= e && dealAmount < e * 1.03);
    if (near) {
      tips.push({
        level: "watch",
        title: `${formatWon(near)} 경계 바로 위입니다`,
        body: `${formatWon(near)} 미만이면 요율이 한 단계 낮습니다. 거래가액 조정 여지가 있다면 보수 차이를 같이 보세요.`,
  ...
  const headline: Headline[] = [
    { label: "중개보수 상한 (부가세 포함, 한쪽 부담)", value: total, hint: `요율 ${(agreed * 100).toFixed(2)}% · 거래금액 ${formatWon(dealAmount)}` },
    { label: "보수", value: fee },
