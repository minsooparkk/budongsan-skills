...
import { hasInvalidNumbers, invalidCalculation, isFiniteNumber, isIntegerInRange, type CalculationValidation } from "./validation";
import { INCOME_BANDS } from "./bands";
import { applyStatute } from "./property";
import { formatWon, truncate10, floorProduct, 만, 억 } from "./won";

/* ═══════════════════════════ 전월세 전환율 ═══════════════════════════ */

/** 시행령 §9: 연 10%, 기준금리 + 2%p 중 낮은 쪽 */
export const CONVERSION_CAP = 0.1;
export const CONVERSION_SPREAD = 0.02;
export function legalConversionRate(baseRate: number): number {
  return Math.min(CONVERSION_CAP, baseRate + CONVERSION_SPREAD);
}
...
export const EMPTY_CONVERSION: ConversionInput = { deposit: 3 * 억, monthly: 0, amount: 1 * 억, direction: "toMonthly", baseRate: 0.025, agreedRate: 0 };

export interface ConversionResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; legalRate: number; rate: number; newDeposit: number; newMonthly: number; }

export function calcConversion(input: ConversionInput): ConversionResult {
  ...
  if (rate < legalRate) steps.push({ label: "합의 전환율", value: `${(rate * 100).toFixed(2)}%` });
  ...
  if (input.direction === "toMonthly") {
    ...
    newDeposit = input.deposit - x; newMonthly = input.monthly + add;
    ...
    newDeposit = input.deposit + add; newMonthly = input.monthly - y;
  ...
  const headline: Headline[] = [
    { label: "전환 후 월세", value: newMonthly, hint: `보증금 ${formatWon(newDeposit)}` },
...
export const RENT_CAP = 0.05;
export interface IncreaseInput { deposit: number; monthly: number; /** 조례 상한 (소수). 0이면 5% */ localCap: number; /** 마지막 증액·계약일부터 지난 개월 */ monthsSince: number; }
export const EMPTY_INCREASE: IncreaseInput = { deposit: 5 * 억, monthly: 0, localCap: 0, monthsSince: 24 };
export interface IncreaseResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; cap: number; maxDeposit: number; maxMonthly: number; allowed: boolean; }
export function calcIncrease(input: IncreaseInput): IncreaseResult {
  if (hasInvalidNumbers(input) || !isIntegerInRange(input.monthsSince, 0, 1200)) return invalidCalculation({ cap: 0, maxDeposit: 0, maxMonthly: 0, allowed: false }, "금액과 계약·증액 후 지난 개월(0~1,200개월 정수)을 확인해 주세요.");
  const steps: Step[] = []; const tips: Tip[] = [];
  const cap = input.localCap > 0 ? Math.min(input.localCap, RENT_CAP) : RENT_CAP;
  ...
  steps.push({ label: "보증금 상한", value: maxDeposit, tone: "sub", note: `증가분 ${formatWon(maxDeposit - input.deposit)}` });
  steps.push({ label: "월세 상한", value: maxMonthly, tone: "sub", note: `증가분 ${formatWon(maxMonthly - input.monthly)}` });
  ...
  return { headline: [ ...(allowed ? [{ label: "올릴 수 있는 보증금 상한", value: maxDeposit, hint: `+${formatWon(maxDeposit - input.deposit)}` }] : [{ label: "현재 증액 청구", value: 0, displayValue: "증액 불가", hint: `${12 - input.monthsSince}개월 뒤 가능` }, { label: "가능 시점 가정 보증금 상한", value: maxDeposit }]), { label: allowed ? "월세 상한" : "가능 시점 가정 월세 상한", value: maxMonthly } ], steps, tips, cap, maxDeposit, maxMonthly, allowed };
}

/* ═══════════════════════════ 임대수익률 ═══════════════════════════ */

export interface YieldInput { price: number; deposit: number; monthly: number; loan: number; loanRate: number; /** 연 경비 (재산세·수선·관리) */ expenses: number; /** 공실률 (소수) */ vacancy: number; /** 취득비용 (취득세·중개보수 등) */ acquisitionCost: number; }
export const EMPTY_YIELD: YieldInput = { price: 5 * 억, deposit: 5000 * 만, monthly: 150 * 만, loan: 2 * 억, loanRate: 0.045, expenses: 300 * 만, vacancy: 0.05, acquisitionCost: 0 };
export interface YieldResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; grossYield: number; netYield: number; equityYield: number; equity: number; cashflow: number; }
export function calcYield(input: YieldInput): YieldResult {
  const steps: Step[] = []; const tips: Tip[] = [];
  ...
  if (input.loan > 0 && netYield < input.loanRate) tips.push({ level: "must", title: "순수익률이 대출금리보다 낮습니다 — 역레버리지", body: `순수익률 ${(netYield * 100).toFixed(2)}% < 대출금리 ${(input.loanRate * 100).toFixed(2)}%. 대출을 늘릴수록 자기자본수익률이 떨어집니다. 시세차익 없이는 손실입니다.` });
...
export interface CompareInput { jeonse: number; wolseDeposit: number; monthly: number; /** 전세대출 금리 */ loanRate: number; /** 대출 비율 (소수) — 나머지는 자기자금 */ loanShare: number; /** 자기자금 기회비용 (예금금리) */ depositRate: number; }
export const EMPTY_COMPARE: CompareInput = { jeonse: 5 * 억, wolseDeposit: 1 * 억, monthly: 120 * 만, loanRate: 0.04, loanShare: 0.6, depositRate: 0.03 };
export interface CompareResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; jeonseMonthly: number; wolseMonthly: number; breakEvenMonthly: number; }
export function calcCompare(input: CompareInput): CompareResult {
  ...
  steps.push({ label: "전세 — 대출이자 (월)", value: jInterest, note: `보증금 ${formatWon(input.jeonse)} 중 ${(input.loanShare * 100).toFixed(0)}% 대출, 금리 ${(input.loanRate * 100).toFixed(2)}%` });
  steps.push({ label: "전세 — 자기자금 기회비용 (월)", value: jOpp, note: `${formatWon(own)} × 예금금리 ${(input.depositRate * 100).toFixed(2)}% ÷ 12` });
  ...
  tips.push({ level: "watch", title: breakEvenMonthly < 0 ? "월세가 0원이어도 전세의 월비용이 낮습니다" : breakEvenMonthly === 0 ? "월세가 0원일 때 두 월비용이 같습니다" : `월세가 ${formatWon(breakEvenMonthly)}보다 낮으면 월세가 유리합니다`, body: breakEvenMonthly < 0 ? "월세 보증금의 기회비용만으로 전세 월비용을 넘습니다. 0원 이상 월세에서는 손익분기점이 없습니다." : "같은 자기자금·대출 조건에서 두 월비용이 같아지는 월세입니다. 그보다 싸면 월세, 비싸면 전세." });
...
export interface InsuranceInput { deposit: number; homePrice: number; seniorDebt: number; apartment: boolean; metro: boolean; /** 보증료율 (소수, 연) */ feeRate: number; years: number; }
export const EMPTY_INSURANCE: InsuranceInput = { deposit: 4 * 억, homePrice: 6 * 억, seniorDebt: 0, apartment: true, metro: true, feeRate: 0.00122, years: 2 };
/** HUG 전세보증금반환보증 — 담보인정비율 90% (2024.1.1~), 보증한도 수도권 7억·그 밖 5억 */
export const LTV_LIMIT = 0.9;
export interface InsuranceResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; eligible: boolean; reasons: string[]; maxDeposit: number; fee: number; }
export function calcInsurance(input: InsuranceInput): InsuranceResult {
  if (hasInvalidNumbers(input) || !isFiniteNumber(input.feeRate, 0, 1) || !isFiniteNumber(input.years, 0, 100)) return invalidCalculation({ eligible: false, reasons: [], maxDeposit: 0, fee: 0 }, "보증금·주택가격·선순위 채권과 보증기간·요율을 확인해 주세요.");
  const steps: Step[] = []; const tips: Tip[] = []; const reasons: string[] = [];
  const limit = input.metro ? 7 * 억 : 5 * 억;
  const collateralLimit = Math.max(0, Math.floor(input.homePrice * LTV_LIMIT - input.seniorDebt));
  const maxDeposit = Math.min(limit, collateralLimit);
  steps.push({ label: "주택가격 × 담보인정비율 90%", value: Math.floor(input.homePrice * LTV_LIMIT), law: "HUG 전세보증금반환보증 업무처리기준 (2024.1.1~)" });
  steps.push({ label: "− 선순위 채권 (근저당 등)", value: input.seniorDebt, tone: "minus" });
  steps.push({ label: "가입 가능한 보증금 상한", value: maxDeposit, tone: "sub" });
  if (input.deposit > collateralLimit) reasons.push(`보증금이 담보 상한 ${formatWon(collateralLimit)}을 넘습니다 (주택가격의 90% − 선순위)`);
  if (input.deposit > limit) reasons.push(`보증금이 ${input.metro ? "수도권" : "그 밖 지역"} 한도 ${formatWon(limit)}을 넘습니다`);
  const eligible = reasons.length === 0;
  const fee = floorProduct([input.deposit, input.feeRate, input.years]);
  ...
  if (!eligible) tips.push({ level: "must", title: "지금 조건으로는 가입이 안 됩니다", body: "보증금을 낮추거나 선순위 근저당을 말소하는 조건을 계약서 특약에 넣으세요. 가입 불가 매물은 반환 위험이 그만큼 큽니다." });
...
export interface OverdueInput { monthly: number; /** 연체 개월 수 */ months: number; /** 지연 일수 (마지막 회차 기준 평균) */ days: number; /** 연 이율 (소수). 약정 없으면 민사 법정이율 5% */ annualRate: number; }
export const EMPTY_OVERDUE: OverdueInput = { monthly: 100 * 만, months: 2, days: 45, annualRate: 0.05 };
export interface OverdueResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; principal: number; interest: number; canTerminate: boolean; }
export function calcOverdue(input: OverdueInput): OverdueResult {
  const steps: Step[] = []; const tips: Tip[] = [];
...
export interface NocInput { deposit: number; monthly: number; maintenance: number; /** 계약면적 (㎡) */ area: number; /** 전용면적 (㎡) */ exclusive: number; /** 전환율 (소수) */ rate: number; /** 렌트프리 개월 (1년 기준) */ rentFree: number; }
export const EMPTY_NOC: NocInput = { deposit: 5000 * 만, monthly: 300 * 만, maintenance: 80 * 만, area: 165, exclusive: 82, rate: 0.06, rentFree: 0 };
export interface NocResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; noc: number; nocPerPyeong: number; nocPerExclusivePyeong: number; effectiveMonthly: number; }
export function calcNoc(input: NocInput): NocResult {
  const steps: Step[] = []; const tips: Tip[] = [];
  ...
  if (freeAdj > 0) steps.push({ label: `− 렌트프리 ${input.rentFree}개월 환산`, value: freeAdj, tone: "minus", depth: 1 });
  ...
  return { headline: [ { label: "환산 월점유비용 (NOC)", value: noc, hint: `전용 평당 ${formatWon(Math.floor(nocPerExclusivePyeong))}` }, { label: "계약 평당", value: Math.floor(nocPerPyeong) }, { label: "전용 평당", value: Math.floor(nocPerExclusivePyeong) } ], steps, tips, noc, nocPerPyeong, nocPerExclusivePyeong, effectiveMonthly };
}

/* ═══════════════════════════ 간주임대료 ═══════════════════════════ */

export const DEEMED_RATE = 0.031;
export const DEEMED_DEDUCTION = 3 * 억;
export const DEEMED_RATIO = 0.6;
export interface DeemedInput { /** 임대 주택 수 (소형주택 제외 후) */ houses: number; /** 2주택일 때: 기준시가 12억 초과 주택 수 */ highValueHouses: number; deposits: number; /** 임대사업 이자·배당 수입 */ interestIncome: number; rate: number; days: number; }
export const EMPTY_DEEMED: DeemedInput = { houses: 3, highValueHouses: 0, deposits: 8 * 억, interestIncome: 0, rate: DEEMED_RATE, days: 365 };
export interface DeemedResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; taxable: boolean; reason: string; deemed: number; }
export function calcDeemed(input: DeemedInput): DeemedResult {
  const steps: Step[] = []; const tips: Tip[] = [];
  let taxable = false; let reason = "";
  if (input.houses >= 3 && input.deposits > 3 * 억) { taxable = true; reason = "3주택 이상 · 보증금 합계 3억원 초과"; }
  else if (input.houses === 2 && input.highValueHouses >= 2 && input.deposits > 12 * 억) { taxable = true; reason = "2주택(기준시가 12억 초과 주택) · 보증금 합계 12억원 초과"; }
  else if (input.houses === 2 && input.highValueHouses < 2) reason = "2주택은 두 채 모두 기준시가 12억을 넘을 때만 셉니다";
  else if (input.houses >= 3) reason = "보증금 합계 3억원 이하";
  else if (input.houses === 2) reason = "보증금 합계 12억원 이하";
  else reason = "1주택은 대상이 아닙니다 (고가주택 월세만 과세)";
  steps.push({ label: "과세 대상 판정", value: taxable ? `대상 — ${reason}` : `대상 아님 — ${reason}`, law: "소득세법 제25조 제1항 · 시행령 제53조 제1항" });
  let deemed = 0;
  if (taxable) {
    const excess = Math.max(0, input.deposits - DEEMED_DEDUCTION);
    const gross = Math.floor((excess * DEEMED_RATIO * input.rate * input.days) / 365);
    deemed = Math.max(0, gross - input.interestIncome);
    steps.push({ label: "보증금 합계", value: input.deposits, tone: "sub" });
    steps.push({ label: "− 3억원", value: DEEMED_DEDUCTION, tone: "minus", law: "소득세법 시행령 제53조 제3항 제1호" });
    steps.push({ label: "× 60%", value: Math.floor(excess * DEEMED_RATIO) });
    steps.push({ label: `× 정기예금이자율 ${(input.rate * 100).toFixed(1)}% × ${input.days}/365`, value: gross, law: "소득세법 시행규칙 제23조 제1항 (2026년 3.1%)" });
    if (input.interestIncome > 0) steps.push({ label: "− 임대사업 이자·배당 수입", value: input.interestIncome, tone: "minus", note: "장부를 쓰는 경우만. 추계신고는 빼지 않는다" });
  ...
  if (taxable) tips.push({ level: "must", title: "간주임대료는 월세와 합쳐 임대소득세로 갑니다", body: `이 ${formatWon(deemed)}에 월세 수입을 더한 총수입이 2천만 이하면 분리과세 14%를 고를 수 있습니다. 주택임대소득세 계산기로 이어서 보세요.`, law: "소득세법 제64조의2" });
...
export const SEPARATE_LIMIT = 2000 * 만;
export interface RentalTaxInput { /** 연 월세 수입 */ rent: number; /** 간주임대료 */ deemed: number; registered: boolean; /** 주택임대 외 종합소득금액 */ otherIncome: number; /** 종합과세 시 실제 필요경비 (장부). 0이면 분리과세 경비율 적용 */ actualExpenses: number; }
export const EMPTY_RENTAL_TAX: RentalTaxInput = { rent: 1800 * 만, deemed: 0, registered: false, otherIncome: 5000 * 만, actualExpenses: 0 };
export interface RentalTaxResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; revenue: number; canSeparate: boolean; separateTax: number; combinedTax: number; better: "separate" | "combined"; total: number; }
export function calcRentalTax(input: RentalTaxInput): RentalTaxResult {
  const steps: Step[] = []; const tips: Tip[] = [];
  const revenue = Math.max(0, input.rent) + Math.max(0, input.deemed);
  const canSeparate = revenue <= SEPARATE_LIMIT;
  const expRate = input.registered ? 0.6 : 0.5;
  const basic = input.otherIncome <= 2000 * 만 ? (input.registered ? 400 * 만 : 200 * 만) : 0;
  ...
  if (canSeparate) {
    ...
    const tax = truncate10(income * 0.14);
    const local = truncate10(tax * 0.1);
  ...
  const withTax = applyStatute(input.otherIncome + rentalIncome, INCOME_BANDS).tax;
  const withoutTax = applyStatute(input.otherIncome, INCOME_BANDS).tax;
  const delta = truncate10(withTax - withoutTax);
  const combinedTax = delta + truncate10(delta * 0.1);
  ...
  steps.push({ label: `종합과세 — 다른 소득 ${formatWon(input.otherIncome)}에 합산한 세액 증분 + 지방소득세`, value: combinedTax, tone: "sub", law: "소득세법 제55조", note: "소득공제·세액공제는 반영하지 않은 증분" });
  ...
  if (canSeparate) tips.push({ level: "save", title: better === "separate" ? `분리과세가 ${formatWon(combinedTax - separateTax)} 유리합니다` : `종합과세가 ${formatWon(separateTax - combinedTax)} 유리합니다`, body: "총수입 2천만 이하는 해마다 둘 중 하나를 고를 수 있습니다. 다른 소득이 적은 해에는 종합과세가 낮을 수 있으니 매년 다시 비교하세요.", law: "소득세법 제64조의2 제1항" });
  if (!input.registered) tips.push({ level: "watch", title: "등록임대주택이면 경비율 60% · 공제 400만", body: "10년 이상 임대 조건입니다. 지키지 못하면 차액을 추징합니다. 5% 임대료 상한도 같이 붙습니다.", law: "소득세법 제64조의2 제3항" });
  ...
  return { headline: [ { label: better === "separate" ? "주택임대소득세 (분리과세)" : "주택임대소득세 (종합과세 증분)", value: total, hint: `총수입 ${formatWon(revenue)} · 지방소득세 포함` }, { label: "분리과세", value: canSeparate ? separateTax : 0, hint: canSeparate ? undefined : "2천만 초과 — 불가" }, { label: "종합과세 증분", value: combinedTax } ], steps, tips, revenue, canSeparate, separateTax, combinedTax, better, total };
}

/* ═══════════════════════════ 명도비용 ═══════════════════════════ */

export interface EvictionInput { /** 전용면적 (㎡) */ area: number; /** 집행 노무비 단가 (원/평) */ laborPerPyeong: number; /** 집행관 수수료·여비 */ bailiffFee: number; /** 사다리차·운반 */ transport: number; /** 창고 보관 월 단가 */ storageMonthly: number; storageMonths: number; /** 소송비용 (인지·송달·변호사) */ litigation: number; /** 명도 지연 개월 — 그동안 못 받는 월세 */ delayMonths: number; monthly: number; }
export const EMPTY_EVICTION: EvictionInput = { area: 84, laborPerPyeong: 120_000, bailiffFee: 200_000, transport: 500_000, storageMonthly: 300_000, storageMonths: 3, litigation: 1_500_000, delayMonths: 6, monthly: 100 * 만 };
export interface EvictionResult extends CalculationValidation { headline: Headline[]; steps: Step[]; tips: Tip[]; execution: number; storage: number; lostRent: number; total: number; }
export function calcEviction(input: EvictionInput): EvictionResult {
  const steps: Step[] = []; const tips: Tip[] = [];
  const pyeong = input.area / 3.3058;
  ...
  steps.push({ label: `강제집행 노무비 (${pyeong.toFixed(1)}평 × ${formatWon(input.laborPerPyeong)})`, value: labor, note: "집행관 사무소가 짐 양·층수·엘리베이터 유무로 인원을 정한다. 실무 단가 평당 10~15만원" });
  ...
  return { headline: [ { label: "명도 총비용", value: total, hint: `직접비 ${formatWon(input.litigation + execution + storage)} + 못 받는 월세 ${formatWon(lostRent)}` }, { label: "집행비용", value: execution }, { label: "소송비용", value: input.litigation } ], steps, tips, execution, storage, lostRent, total };
}
