...
import { formatWon, pct, truncate10, 억, 만 } from "./won";
...
export const EMPTY_INHERITANCE: InheritanceInput = {
  year: 2026,
  month: 8,
  house: 12 * 억,
  land: 0,
 ...
 */
export function funeralDeduction(spent: number): number {
  return Math.min(Math.max(spent, 500 * 만), 1000 * 만);
}

/** 배우자 법정상속분 — 민법 §1009 (배우자 1.5 : 자녀 각 1) */
export function spouseLegalShare(children: number): number {
  if (children <= 0) return 1; // 자녀가 없으면 배우자가 사실상 전부 (직계존속 유무는 묻지 않는다)
...
export function calcInheritance(input: InheritanceInput): InheritanceResult {
  ...
  if (input.house) push({ label: "주택", value: input.house, depth: 1 });
  if (input.land) push({ label: "토지", value: input.land, depth: 1 });
  if (input.deposit) push({ label: "예금·적금", value: input.deposit, depth: 1 });
  ...
  push({
    ...
    note:
      input.funeral < 500 * 만
        ...
        : input.funeral > 1000 * 만
          ? `실제 ${formatWon(input.funeral)}을 썼어도 1,000만원에서 잘린다`
          : undefined,
  });
  if (subtractRaw > gross) {
    push({
      label: "차감 한도 적용",
      value: `${formatWon(subtractRaw)} → ${formatWon(subtract)}`,
      tone: "plain",
      law: "상증세법 제14조 제1항",
  ...
  const basic = 2 * 억;
  const childDed = input.children * 5000 * 만;
  const minorDed = input.minors * 1000 * 만 * 10; // 평균 잔여 10년으로 가정 — 화면에 밝힌다
  const elderDed = input.elders * 5000 * 만;
  const disabledDed = Math.max(0, input.disabledYears) * 1000 * 만;
  ...
  push({
    ...
    note: spouseOnly
      ...
      : lumpSumUsed
        ? `인적공제 합계 ${formatWon(personalSum)}보다 5억원이 커서 일괄공제를 택했다`
  ...
  if (input.hasSpouse) {
    ...
    spouseLimit = Math.min(spouseBase * share, 30 * 억);
    spouseActual = input.spouseActual ?? (gross - Math.min(input.debt, gross)) * share;
    spouseDeduction = Math.max(Math.min(spouseActual, spouseLimit), 5 * 억);
    push({
      label: "배우자 상속공제",
      ...
      note:
        spouseDeduction === 5 * 억
          ...
          : `법정상속분 ${pct(share, 1)} 한도 ${formatWon(spouseLimit)} 이내`,
    });
  }
  ...
  if (netFinancial > 2000 * 만) {
    financialDeduction = Math.min(Math.max(netFinancial * 0.2, 2000 * 만), 2 * 억);
  } else {
  ...
  if (financialDeduction > 0) {
    push({
      ...
      tone: "minus",
      law: "상증세법 제22조",
      note: `순금융재산 ${formatWon(netFinancial)}의 20%${
        financialDeduction === 2 * 억 ? " — 2억원 한도에서 잘림" : ""
      }`,
    });
  }
  ...
  if (input.coResidence && input.house > 0) {
    coResidenceDeduction = Math.min(
      Math.max(0, input.house - input.houseSecuredDebt),
      6 * 억,
    );
    push({
      label: "동거주택 상속공제",
  ...
  const deductionRaw =
    baseDeduction + spouseDeduction + financialDeduction + coResidenceDeduction;

  // 공제 적용의 한도 (§24) — 과세가액에 가산한 사전증여재산은 공제 밑천이 되지 않는다
  const limitCut =
    taxableEstate > 5 * 억 ? Math.max(0, input.priorGift) : 0;
  ...
  if (deduction < deductionRaw) {
    push({
      label: "공제 적용의 한도",
      value: `${formatWon(deductionRaw)} → ${formatWon(deduction)}`,
      tone: "plain",
      law: "상증세법 제24조",
  ...
  push({ label: "적용세율", value: pct(applied.rate, 0), law: "상증세법 제26조" });
  ...
  if (input.priorGiftTaxPaid > 0 && grossTax > 0 && taxableEstate > 0) {
    ...
    push({
      ...
      law: "상증세법 제28조",
      note:
        giftCredit < input.priorGiftTaxPaid
  ...
  const payable = truncate10(afterCredit - filingCredit);
  ...
  const headline: Headline[] = [
    { label: "납부할 상속세", value: payable, hint: `실효세율 ${pct(effectiveRate, 1)}` },
...
export function filingDeadline(year: number, month: number): string {
...
function inheritanceTips(
  ...
  if (r.payable > 20_000_000) {
    tips.push({
      ...
      body: `납부세액이 2천만원을 넘으면 담보를 제공하고 연부연납을 신청할 수 있다. 지금 세액 ${formatWon(
        r.payable,
      )}이면 대상이다. 다만 연부연납 가산금(이자)이 붙는다.`,
      law: "상증세법 제71조·제72조",
    });
  }
  ...
  if (input.hasSpouse && input.spouseActual === null) {
    tips.push({
      ...
      body: `지금은 법정상속분대로 나눈다고 보고 ${formatWon(
        r.spouseDeduction,
      )}을 공제했다. 배우자가 실제로 더 받으면 공제가 늘지만, 그 재산은 나중에 2차 상속으로 다시 과세된다. 두 번의 상속을 함께 봐야 답이 나온다.`,
      law: "상증세법 제19조",
    });
  }
